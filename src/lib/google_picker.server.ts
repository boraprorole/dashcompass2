...
export async function listGoogleAdsCustomersForReport(userId: string, reportId: string) {
  ...
  if (!listRes.ok) {
    ...
    if (listRes.status === 404) {
      ...
      throw new Error(`Ads list: 404. O Google Ads API retornou 404. Isso geralmente indica que o Developer Token não é válido para a versão v25 ou a conta Google não tem acesso ao Google Ads.`);
    }
