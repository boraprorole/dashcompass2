  ...
  const { data: conns, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [ga, gsc, gads] = await Promise.all([
        supabase.from("ga_connections").select("id, google_email, ga_property_id").eq("report_id", reportId),
        supabase.from("gsc_connections").select("id, google_email, site_url").eq("report_id", reportId),
        supabase.from("google_ads_connections").select("id, google_email, customer_id").eq("report_id", reportId),
      ]);
      return {
        ga: ga.data || [],
        gsc: gsc.data || [],
