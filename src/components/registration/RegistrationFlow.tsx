...
/** Plano tal como configurado no Admin Global. */
...
export function RegistrationFlow({ lang }: RegistrationFlowProps) {
  ...
  const isPT = lang === "pt";
  const currency: "brl" | "usd" = isPT ? "brl" : "usd";
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .select("key, label, value_brl, value_usd, features_pt, features_en, sort_order")
        .eq("active", true)
  ...
  const t = useMemo(
    () => ({
      ...
      subtitle: isPT
        ? "Dados básicos para sua agência no DashCompass."
        : "Basic data for your agency on DashCompass.",
      ...
      plansSubtitle: isPT
        ? "Selecione a melhor opção para sua operação."
        : "Select the best option for your operation.",
      perMonth: isPT ? "/mês" : "/mo",
      paymentTitle: isPT ? "Pagamento" : "Payment",
      paymentSubtitle: isPT
        ? "Finalize sua assinatura com segurança."
        : "Complete your subscription securely.",
      noPlans: isPT ? "Nenhum plano disponível no momento." : "No plans available right now.",
    }),
    [isPT],
  );
  ...
  const priceLabel = (plan: PricingPlan) =>
    isPT
      ? `R$ ${Number(plan.value_brl).toLocaleString("pt-BR")}`
      : `$${Number(plan.value_usd).toLocaleString("en-US")}`;

  const featuresOf = (plan: PricingPlan) =>
    (isPT ? plan.features_pt : plan.features_en) ?? [];
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // O checkout é embutido nesta mesma página: não há redirecionamento externo,
      // então uma sessão ainda não confirmada nunca derruba o usuário no /login.
            ...
            {step === "account" ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <Label
                        htmlFor="displayName"
                        className="text-xs font-bold text-white/50 ml-1 uppercase tracking-widest"
                      >
                        {t.labelName}
                      </Label>
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        {t.nextStep} <ArrowRight className="ml-2 h-5 w-5" />
                      </>
                    )}
                  </Button>
                </form>
              ...
              >
                ...
                <div className="space-y-4">
                  {plansLoading ? (
                    ...
                    <p className="text-center text-white/40 py-8">{t.noPlans}</p>
                  ) : (
                    plans.map((plan) => (
                      <button
                        key={plan.key}
                      ...
                      >
                        <div className="flex justify-between items-center mb-2">
                          <span
                            className={cn(
                              "font-bold text-lg",
                              selectedPlan === plan.key ? "text-primary" : "text-white",
                            )}
                          >
                            {plan.label}
                          </span>
                          <span className="font-bold text-white">
                            {priceLabel(plan)}
                            <span className="text-xs text-white/40 font-normal">{t.perMonth}</span>
                          </span>
                        </div>
                        <div className="flex flex-wrap gap-x-4 gap-y-1">
                          {featuresOf(plan).map((f, i) => (
              ...
              >
                <div className="text-center mb-6">
                  <h1 className="text-3xl font-bold text-white mb-2">{t.paymentTitle}</h1>
                  <p className="text-white/40">{t.paymentSubtitle}</p>
                </div>

                <div id="checkout" className="rounded-2xl overflow-hidden bg-white p-2">
                  {clientSecret && (
