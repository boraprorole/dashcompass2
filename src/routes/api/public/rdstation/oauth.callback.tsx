...
import { exchangeRdCode, saveRdConnection, verifyRdState } from "@/lib/rdstation.server";

function html(body: string, status = 200) {
  return new Response(
    `<!doctype html><html><head><meta charset="utf-8"><title>RD Station</title>
...
export const Route = createFileRoute("/api/public/rdstation/oauth/callback")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        if (error)
          return html(
            `<h1>Autorização negada</h1><p>${error}</p><a href="/admin">Voltar</a>`,
            400,
          );
        if (!code || !state)
          return html(`<h1>Parâmetros ausentes</h1><a href="/admin">Voltar</a>`, 400);
        try {
          const parsed = (await verifyRdState(state)) as { reportId?: string };
          if (!parsed.reportId) throw new Error("state incompleto");
          const tokens = await exchangeRdCode(code);
          await saveRdConnection({
            reportId: parsed.reportId,
            accountName: null,
            tokens,
          });
          return html(
            `<h1>RD Station conectado</h1><p>Conta vinculada ao relatório. Volte ao admin para visualizar as métricas.</p><a href="/admin">Voltar ao admin</a>`,
          );
        } catch (e) {
          return html(
